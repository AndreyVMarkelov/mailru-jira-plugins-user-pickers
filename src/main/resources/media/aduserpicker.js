...
function initSettingsDlg(baseUrl, cfId) {
	...
	jQuery.ajax({
		url : baseUrl
				+ "/rest/rolegroupusercfws/1.0/usrpickerssrv/initsettingsdlg",
		type : "POST",
		dataType : "json",
		data : {
			"cfId" : cfId
...
function initSelectedSettingsDlg(baseUrl, cfId) {
	...
	jQuery
			.ajax({
				url : baseUrl
						+ "/rest/rolegroupusercfws/1.0/usrpickerssrv/initselectedsettingsdlg",
				type : "POST",
				dataType : "json",
				data : {
					"cfId" : cfId
...
function configureSingleField(event, baseUrl, cfId) {
	...
	md
			.addButton(
					AJS.I18n.getText("jrole-group-usercf.applyjqlbtn"),
					function() {
						jQuery
								.ajax({
									url : baseUrl
											+ "/rest/rolegroupusercfws/1.0/usrpickerssrv/configuresingle",
									type : "POST",
									dataType : "json",
									async : false,
									data : AJS.$("#singlesettingsform")
											.serialize(),
			...
			.addButton(
					...
					function() {
						jQuery
								.ajax({
									url : baseUrl
											+ "/rest/rolegroupusercfws/1.0/usrpickerssrv/confselectedsettingsdlg",
									type : "POST",
									dataType : "json",
									data : AJS.$("#singlesettingsselform")
											.serialize(),
									async : false,
									error : function(xhr, ajaxOptions,
											thrownError) {
...
function addGroup(dataInputId, listDivId, groupsSelectId) {
	...
	var newElem = jQuery("<div id='"
			+ grId
			+ "'><span>"
			+ AJS.format(AJS.I18n.getText("jrole-group-usercf.share_group"),
					jQuery(group).text()) + "</span></div>");
	jQuery("#" + listDivId).append(newElem);
	jQuery("#" + listDivId + " #share_trash_sh").eq(0).clone().attr('id', '')
			.show().appendTo(newElem);
}
...
function addProject(dataInputId, listDivId, projectsSelectId, rolesSelectId) {
	...
	var textVal;
	if (jQuery(role).val()) {
		textVal = AJS.format(AJS.I18n
				.getText("jrole-group-usercf.share_project_role"), jQuery(proj)
				.text(), jQuery(role).text());
	} else {
		textVal = AJS.format(AJS.I18n
				.getText("jrole-group-usercf.share_project"), jQuery(proj)
				.text());
	}

	var newElem = jQuery("<div id='" + prId + "'><span>" + textVal
			+ "</span></div>");
	jQuery("#" + listDivId).append(newElem);
	jQuery("#" + listDivId + " #share_trash_sh").eq(0).clone().attr('id', '')
			.show().appendTo(newElem);
...
function selectAndReturnValue(value, returnId) {
	...
	if (returnElem.length > 0) {
		var tagNameStr;
		if (returnElem.attr('tagName')) {  // Compability for different jQuery versions
			tagNameStr = returnElem.attr('tagName').toString();
		} else if (returnElem.prop('tagName')) {
			tagNameStr = returnElem.prop('tagName').toString();
		}
		
		if ('INPUT' == tagNameStr) {
			returnElem.val(value);
			returnElem.trigger('input');
		} else if ('SELECT' == tagNameStr) {
