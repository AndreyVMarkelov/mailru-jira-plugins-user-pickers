...
function upChooseValFromNewWindow(cfId, baseUrl, returnCfId, isRestricted) {
	var marginTop = 100;
	var marginLeft = 500;
	var pickerWindow;
	...
	if (!isRestricted) {
		pickerWindow = window
				.open(
						baseUrl
								+ "/secure/popups/UserPickerBrowser.jspa?formName=jiraform&multiSelect=false&decorator=popup&element="
								+ cfId,
						AJS.I18n.getText("jrole-group-usercf.userpicker.title"),
						"status=0,toolbar=0,location=0,menubar=0,directories=0,resizable=1,scrollbars=1,height=800,width=900");
	} else {
		pickerWindow = window
				.open(
						baseUrl
